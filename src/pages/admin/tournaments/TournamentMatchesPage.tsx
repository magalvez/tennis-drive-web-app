...
const TournamentMatchesPage = () => {
    ...
    const handleGenerateGroups = async () => {
        ...
        const count = window.prompt("How many groups?", "4");
        if (!count) return;
    ...
    const handleGenerateKnockout = async () => {
        ...
        try {
            ...
            if (qualifiers.length < 2) {
                alert("Not enough qualified players. Finalize groups first.");
                return;
            }
    ...
    const handleFinalizeGroup = async (groupName: string) => {
        if (!id) return;
        const count = window.prompt("Qualifiers from this group?", "2");
        if (!count) return;
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-gray-500 font-bold uppercase text-xs tracking-widest mt-1">Match Management Center</p>
                    </div>
                </div>
                    ...
                    <div className="space-y-12">
                        {Object.keys(matchesByGroupName).length === 0 ? (
                            <div className="glass p-20 rounded-[40px] border-dashed border-2 border-white/10 flex flex-col items-center justify-center text-center no-print">
                                ...
                                <h3 className="text-white text-2xl font-bold mb-2">No Groups Generated Yet</h3>
                                <p className="text-gray-500 max-w-md mb-10">Start the tournament by assigning players to groups and generating the round-robin matches.</p>
                                <button
                                        ...
                                        <div key={groupName} className="space-y-6">
                                            <div className="flex items-center justify-between px-4">
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    <div>
                                                        <h3 className="text-white text-xl font-bold">Group {groupName}</h3>
                                                ...
                                                <div className="flex items-center gap-3 no-print">
                                                    {!isFinalized ? (
                                                        ...
                                                        <button
                                                            onClick={async () => {
                                                                if (window.confirm("Undo finalization?")) {
                                                ...
                                                {matchesByGroupName[groupName].map(match => (
                                                    ...
                                                    >
                                                        <div className="flex items-center justify-between mb-2">
                                                            <span className="text-[10px] font-black uppercase text-gray-600 tracking-tighter">Match #{match.id.substring(0, 4)}</span>
                                                        ...
                                                        <div className="pt-3 border-t border-white/5 flex justify-center bg-white/5 -mx-5 -mb-5 rounded-b-3xl py-2">
                                                            <span className="text-tennis-green font-black text-xs">
                                                                {match.status === 'completed' ? (match.sets ? formatMatchScore(match.sets) : match.score) : 'SCHEDULED'}
                                                            </span>
                                                        </div>
                                                    </div>
                                                ))}
                        ...
                        <div className="flex justify-center pt-10 border-t border-white/5 gap-4 no-print">
                            <button
                                onClick={async () => {
                                    if (window.confirm("Reset entire group stage? This will delete all matches.")) {
                        ...
                        <div className="glass rounded-[40px] border-white/5 overflow-hidden">
                            <table className="w-full text-left">
                                <thead className="bg-white/5 border-b border-white/5">
                                    <tr>
                                        <th className="px-8 py-6 text-[10px] font-black uppercase text-gray-500 tracking-widest">Player</th>
                                        <th className="px-4 py-6 text-[10px] font-black uppercase text-gray-500 tracking-widest text-center">Group</th>
                                        <th className="px-4 py-6 text-[10px] font-black uppercase text-gray-500 tracking-widest text-center">P</th>
                                        <th className="px-4 py-6 text-[10px] font-black uppercase text-gray-500 tracking-widest text-center">W</th>
                                        <th className="px-4 py-6 text-[10px] font-black uppercase text-gray-500 tracking-widest text-center">L</th>
                                        <th className="px-8 py-6 text-[10px] font-black uppercase text-tennis-green tracking-widest text-right">Points</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-white/5">
                                    ...
                                    {standings.length === 0 && (
                                        <tr>
                                            <td colSpan={6} className="px-8 py-20 text-center text-gray-500 font-bold uppercase text-xs tracking-widest">No data available for this category</td>
                                        </tr>
                                    )}
                                </tbody>
                            </table>
                    ...
                    <div className="space-y-10">
                        {bracketMatches.length === 0 ? (
                            <div className="glass p-20 rounded-[40px] border-dashed border-2 border-white/10 flex flex-col items-center justify-center text-center no-print">
                                ...
                                <h3 className="text-white text-2xl font-bold mb-2">No Brackets Found</h3>
                                <p className="text-gray-500 max-w-md mb-10">Once the group stage is complete and groups are finalized, you can generate the elimination bracket.</p>
                                <button
                                    onClick={handleGenerateKnockout}
                                    disabled={processing}
                            ...
                            <div className="space-y-12">
                                ...
                                <div className="flex justify-center border-t border-white/5 pt-10 no-print">
                                    <button
                                        onClick={async () => {
                                            if (window.confirm("Delete all bracket matches?")) {
                ...
                <>
                    ...
                    <div className="fixed right-0 top-0 h-full w-full max-w-xl bg-gray-950 border-l border-white/10 z-50 p-12 overflow-y-auto transform transition-transform duration-300 animate-slide-in-right no-print">
                        <div className="flex justify-between items-center mb-10">
                            <div>
                                <h2 className="text-white text-3xl font-black uppercase tracking-tight">Enter Match Score</h2>
                                <p className="text-gray-500 text-xs font-bold uppercase tracking-widest mt-1">Official Result Entry</p>
                            </div>
                        ...
                        <div className="space-y-12">
                            ...
                            <div className="space-y-6">
                                <h3 className="text-white text-lg font-bold">Set Results</h3>
                                {[0, 1, 2].map((i) => (
                                    <div key={i} className="flex items-center gap-6">
                                        <div className="w-12 h-12 bg-white/5 rounded-xl flex items-center justify-center text-gray-500 font-black text-xs">Set {i + 1}</div>
                                ...
                                >
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div className="text-left">
                                            <p className="font-bold text-sm uppercase tracking-tight leading-none">Declare Withdrawal</p>
                                            <p className="text-[10px] font-bold mt-1 opacity-60">Victory by default/walkover</p>
                                        </div>
                                    </div>
                            ...
                            {/* Winner Selection */}
                            <div className="space-y-4">
                                <p className="text-gray-500 text-[10px] font-black uppercase tracking-widest text-center">Declared Winner</p>
